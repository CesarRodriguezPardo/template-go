...
func CreateCatService(cat *models.Cat) (*models.Cat, error) {
	if cat.Name == "" {
	...
	if cat.Owner == primitive.NilObjectID {
		return nil, errors.New("No existe dueño del gatito/a.")
	}

	id, err := catRepo.InsertOne(cat)
...
func FindAllCatService() ([]*models.Cat, error) {
	cats, err := catRepo.FindAll()
